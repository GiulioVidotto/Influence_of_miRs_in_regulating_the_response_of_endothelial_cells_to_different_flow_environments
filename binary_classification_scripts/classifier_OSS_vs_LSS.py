...
file_path = os.getcwd() + "/binary_classification_scripts/OSS_vs_LSS_classifier_data_final.csv"
...
C = [0.001, 0.005, 0.05, 0.01, 0.1, 1, 10]
...
n_estimators = [int(x) for x in np.linspace(start = 200, stop = 600, num= 5)]  # Number of trees
learning_rate = [0.005, 0.01]  # Learning rate
max_depth = range(2,4)  # Maximum depth of individual trees
min_samples_split = range(8,10)  # Minimum number of samples required to split a node
